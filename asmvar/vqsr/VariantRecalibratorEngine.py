...
class VariantRecalibratorEngine:
    ...
    def ClassifyData(self, dataSize): 
        ...
        testSetIdx  = range(cvSetSize + trainSetSize, dataSize)
    ...
    def GenerateModel(self, data, maxGaussians):
        ...
        minBIC, bics = np.inf, []
        for g in gmms: 
            print >> sys.stderr, ('[INFO] Trying %d gaussian in GMM process '
            ...
            print >> sys.stderr, ('  -- Converge infomation of training '
                                  'process:', g.converged_)
        print >> sys.stderr, '[INFO] All the BIC:', bics
        print >> sys.stderr, ('[INFO] Model Training Done. And take the model '
    ...
    def EvaluateData(self, data, gmm, evaluateContrastively = False):
        ...
        print >> sys.stderr, '[INFO] Evaluating full set of %d variants ...' % len(data)
        for i,_ in enumerate(data): 
            ...
            if evaluateContrastively: 
                ...
                if thisLod  == float('inf'): 
                    data[i].lod = self.MIN_ACCEPTABLE_LOD_SCORE * (1.0 + 
                                    np.random.rand(1)[0])
            else:
